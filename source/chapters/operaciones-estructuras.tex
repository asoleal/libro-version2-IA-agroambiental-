...
\begin{itemize}
    \item \textbf{Regresión Lineal:} Los coeficientes óptimos se estiman como $\beta = (\mathbf{X}^\top \mathbf{X})^{-1} \mathbf{X}^\top \mathbf{y}$.
    \item \textbf{Distancia de Mahalanobis:} $\sqrt{(\mathbf{x}-\mu)^\top \mathbf{\Sigma}^{-1} (\mathbf{x}-\mu)}$. Usada para detectar outliers multivariados (ej. animales enfermos con patrones fisiológicos atípicos).
...
\begin{ingebox}{Rendimiento en Deep Learning}
...
\begin{alertblock}{Advertencia de Memoria}
